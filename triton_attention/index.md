...
- 它由**多个全连接层（Dense Layer）**堆叠而成，通常包括：
  - 输入层
  - 一个或多个**隐藏层**（每个隐藏层后常接非线性激活函数，如 ReLU）
  - 输出层
- 特点：**前馈**（无环、无反馈）、**全连接**、**非线性激活**。
- 用途：可用于分类、回归等任务。

### 2. **FFN（前馈网络）**
- FFN 是一个更广义的术语，泛指**所有前馈结构的神经网络**（即信号单向从前向后传播，不形成循环）。
- **MLP 是 FFN 的一个特例**。
- 在现代 Transformer 架构中，"FFN layer" 通常指一个**两层全连接网络**：
  ```text
  FFN(x) = max(0, xW₁ + b₁)W₂ + b₂
  ```
  即：线性变换 → ReLU（或 GELU）→ 线性变换。
- 这种 FFN 层**结构上就是一个小型 MLP（通常只有1个隐藏层）**，嵌入在 Transformer 的每个 Encoder/Decoder block 中。

---

### 主要区别总结：

| 方面 | MLP | FFN（特指 Transformer 中的 FFN 层） |
|------|-----|-----------------------------------|
| 范畴 | 一种具体的网络结构 | 一类网络（广义）或一个模块（狭义） |
| 结构 | 可含多个隐藏层 | 通常只有**1个隐藏层**（两层线性） |
| 使用场景 | 独立模型（如分类器） | 作为**子模块**嵌入大模型（如 Transformer） |
| 激活函数 | ReLU、tanh 等 | ReLU 或 GELU（Transformer 常用） |

> 简单说：在 Transformer 中所说的 "FFN layer"，本质上就是一个**浅层 MLP**（通常2层），而 MLP 本身也可以看作是由多个 FFN 层堆叠而成的网络。

如果你在阅读 Transformer 相关论文，看到 “FFN”，基本等价于一个带激活函数的两层全连接网络，这是 MLP 的一个简化特例。
